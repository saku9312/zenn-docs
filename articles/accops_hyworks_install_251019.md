...
## 11/03追記
- 物理マシン（別セグメントのノートPC）でもSession Hostをインストールすることで、仮想マシンと同じようにVDI対象にできた。
- ただ、後から追加したマシンに自動接続され、仮想マシンには接続できず。
- スペックなどで自動的に振り分けされるコネクションブローカーの立ち位置しかできないのか、それとも何か優先順位の並べ替えや手動選択ができるのか、検証期間は残り少ないが確認しておきたい。